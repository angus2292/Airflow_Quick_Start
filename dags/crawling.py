...
def crawling(**context):
    ...
    task_instance.xcom_push(key='web_crawling_result', value=df)
    return df

def processing_data(**context):
    df = context['task_instance'].xcom_pull(task_ids="web_crawling", key='web_crawling_result')
    ...
    task_instance = context['task_instance']
    task_instance.xcom_push(key='chunkdata_result', value=df)

    return df

def write_table(**context):
    df = context['task_instance'].xcom_pull(task_ids="chunkdata", key='chunkdata_result')
...
dag = DAG(
    'ptt_stock_web_crawling_v0',
    default_args=default_args,
    schedule_interval = timedelta(days=1)
)

web_crawling = PythonOperator(
    task_id = 'web_crawling',
    python_callable = crawling,
    provide_context=True,
    dag=dag
)

chunk_data = PythonOperator(
    task_id = 'chunkdata',
...
web_crawling >> chunk_data >> insert_table
